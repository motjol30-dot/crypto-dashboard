...
// ── تجميعات بسيطة لكل مجموعة (بيانات مشتركة تُستخدم بمربع "التحليل" وبإشارتي الارتفاع/النزول المبكر) ──
function tallyStability(indicators) {
  let bull = 0, bear = 0;
  if (indicators.adx && indicators.adx.adx >= 20) { if (indicators.adx.pdi > indicators.adx.mdi) bull++; else bear++; }
  if (indicators.williamsR) { if (indicators.williamsR.zoneUp) bull++; if (indicators.williamsR.zoneDown) bear++; }
  if (indicators.ichimoku && indicators.currentPrice != null) {
...
function tallyReversal(indicators) {
  let bull = 0, bear = 0;
  if (indicators.stochRsi) { if (indicators.stochRsi.crossUp || indicators.stochRsi.zoneUp) bull++; if (indicators.stochRsi.crossDown || indicators.stochRsi.zoneDown) bear++; }
  if (indicators.williamsR) { if (indicators.williamsR.crossUpFrom80 || indicators.williamsR.zoneUp) bull++; if (indicators.williamsR.crossDownFrom20 || indicators.williamsR.zoneDown) bear++; }
  if (indicators.bbPercentB) { if (indicators.bbPercentB.zoneUp) bull++; if (indicators.bbPercentB.zoneDown) bear++; }
...
function tallyMomentum(indicators) {
  let bull = 0, bear = 0;
  if (indicators.rsi != null) { if (indicators.rsi > 55) bull++; else if (indicators.rsi < 45) bear++; }
  if (indicators.macd) { if (indicators.macd.value > indicators.macd.signal) bull++; else bear++; }
  if (indicators.stochastic) { if (indicators.stochastic.k > 55 && indicators.stochastic.k >= indicators.stochastic.d) bull++; else if (indicators.stochastic.k < 45 && indicators.stochastic.k <= indicators.stochastic.d) bear++; }
...
function tallyVolume(indicators) {
  let bull = 0, bear = 0;
  if (indicators.cvd && indicators.cvd.signal === 'bullish_divergence') bull++;
  if (indicators.cvd && indicators.cvd.signal === 'bearish_divergence') bear++;
  if (indicators.accDist && indicators.accDist.zone === 'تجميع (Accumulation)') bull++;
...
// ── إشارتا الارتفاع/النزول المبكر: نفس فكرة السهمين باللوحة تمامًا — كل مؤشر يُحسب فقط لو أعطى إشارته
// المطلوبة تحديدًا (صاعدة للارتفاع، هابطة للنزول)، عكس الاتجاه لا يُحسب له إطلاقًا (صفر، مو سالب) ──
function computeEarlySignal(indicators, direction) {
  if (!indicators) return 0;
  const wantBull = direction === 'up';
  let triggered = 0, total = 0;
  const check = (ok) => { total++; if (ok) triggered++; };

  if (indicators.rsiDivergence) check(wantBull ? indicators.rsiDivergence.type === 'bullish' : indicators.rsiDivergence.type === 'bearish');
  if (indicators.rsi != null) check(wantBull ? indicators.rsi < 45 : indicators.rsi > 55);
  if (indicators.stochastic) check(wantBull ? (indicators.stochastic.k < 45 && indicators.stochastic.k <= indicators.stochastic.d) : (indicators.stochastic.k > 55 && indicators.stochastic.k >= indicators.stochastic.d));
  if (indicators.stochRsi) check(wantBull ? (indicators.stochRsi.crossUp || indicators.stochRsi.zoneUp) : (indicators.stochRsi.crossDown || indicators.stochRsi.zoneDown));
  if (indicators.williamsR) check(wantBull ? (indicators.williamsR.crossUpFrom80 || indicators.williamsR.zoneUp) : (indicators.williamsR.crossDownFrom20 || indicators.williamsR.zoneDown));
  if (indicators.obv != null && indicators.obvTrendRef != null) check(wantBull ? indicators.obv > indicators.obvTrendRef : indicators.obv < indicators.obvTrendRef);

  const groups = [tallyReversal(indicators), tallyStability(indicators), tallyVolume(indicators), tallyMomentum(indicators)];
  for (const g of groups) {
    const majority = g.bull > g.bear ? 'bull' : g.bear > g.bull ? 'bear' : 'neutral';
    check(wantBull ? majority === 'bull' : majority === 'bear');
  }

  return total > 0 ? triggered / total : 0; // نطاق 0..1
}

const EARLY_SIGNAL_THRESHOLD = 0.5; // 50% فأكثر من المؤشرات المتاحة تُعتبر "قوية جدًا" — نفس الفكرة بالضبط للشراء والبيع
...
async function evaluateBotSignal(symbol) {
  ...
  const earlyDownSignal = computeEarlySignal(indicators, 'down'); // "السهم الأحمر" — نستخدمه كشرط استعداد للشراء
  const earlyUpSignal = computeEarlySignal(indicators, 'up');     // "السهم الأخضر" — نستخدمه كشرط استعداد للبيع المبكر
...
// البيع المبكر: يلغي أمر جني الربح البعيد المعلّق ويضع بدله أمر Limit أقرب للسعر الحالي — يبقى Limit دائمًا، أبدًا سوق مفتوح
async function placeBotEarlySell(symbol, position, pendingSell, sig) {
  try { await cancelOrder(pendingSell.exchange, symbol, pendingSell.orderId); }
  catch (err) { return; } // فشل الإلغاء (يمكن اتنفذ لتوّه) — نتركه للدورة القادمة تتعامل معه عاديًا

  const sellPrice = roundPrice(sig.price); // أقرب للسعر الحالي بدل انتظار هدف الربح الأبعد
  const data = await placeLimitOrder(botState.exchange, symbol, 'SELL', sellPrice, position.qty);
  if (!data.orderId) {
    // فشل وضع البديل — نرجّع أمر جني الربح الأصلي حتى لا تبقى الصفقة بدون أي أمر بيع معلّق إطلاقًا
    try { await placeBotTakeProfitSell(symbol, position); } catch (err2) { logBotError(symbol, err2); }
    return;
  }
  botState.pendingOrders[symbol] = { orderId: data.orderId, side: 'SELL', price: sellPrice, qty: position.qty, placedAt: Date.now(), exchange: botState.exchange };
  botState.tradeLog.unshift({
    time: Date.now(), symbol, type: 'order', side: 'SELL', price: sellPrice, qty: position.qty, exchange: botState.exchange,
    reason: `بيع مبكر — سهم الارتفاع قوي (${(sig.earlyUpSignal * 100).toFixed(0)}%) والقرار العام لم يعد يدعم الاستمرار`,
  });
  botState.tradeLog = botState.tradeLog.slice(0, 50);
}
...
async function runBotCycle() {
  ...
  for (const symbol of SCAN_SYMBOLS) {
    try {
      ...
      // الشراء: لازم "السهم الأحمر" (النزول المبكر) يكون قوي جدًا أولًا — يعني السوق أصلًا بيّن ضعف/تشبع حقيقي،
      // مو مجرد قرار عام بالشراء وهو بعد بالقمة. هذا يمنع الشراء أعلى السعر بدون تأكيد استعداد حقيقي.
      if (!alreadyOpen && sig.action === 'buy' && sig.passesFilters && sig.earlyDownSignal >= EARLY_SIGNAL_THRESHOLD) {
        candidates.push(sig);
      }

      // البيع المبكر: لو عندنا صفقة مفتوحة وربح فعلي متحقق، و"السهم الأخضر" (الارتفاع المبكر) قوي جدًا،
      // و"القرار النهائي" نفسه ما عاد يدعم استمرار الصعود — نستبدل أمر جني الربح المعلّق بأمر أقرب للسعر
      // الحالي بدل انتظار الهدف الأبعد، بنفس مبدأ عدم استخدام بيع مفتوح إطلاقًا (أمر Limit دائمًا)
      const pos = botState.positions[symbol];
      const pendingSell = botState.pendingOrders[symbol] && botState.pendingOrders[symbol].side === 'SELL' ? botState.pendingOrders[symbol] : null;
      if (pos && pendingSell && sig.price > pos.entryPrice && sig.earlyUpSignal >= EARLY_SIGNAL_THRESHOLD && sig.dashboardSignal < BOT_BUY_THRESHOLD) {
        try { await placeBotEarlySell(symbol, pos, pendingSell, sig); } catch (err) { logBotError(symbol, err); }
      }
