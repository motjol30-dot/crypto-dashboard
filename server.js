...
/* ============================================================
 * بوت التداول التلقائي: مفاتيح API لكل منصة
 * ============================================================ */
...
/* ============================================================
 * نظرة عامة على السوق: 5 مؤشرات مجانية
 * ============================================================ */
...
/* ============================================================
 * الطبقة الكلية: الدولار (DXY) + النفط (WTI)
 * ============================================================ */
...
function computeExplosionScore(candles) {
  ...
  const widthPercentile = maxWidth > minWidth ? (lastWidth - minWidth) / (maxWidth - minWidth) : 0.5;
...
function computeIndicators(candles) {
  ...
  if (rsiArr.length >= 17) {
    ...
    if (kArr.length >= 4) {
      ...
      stochRsi = {
        ...
        zoneUp: kNow < 25, zoneDown: kNow > 75,
      };
    }
  }
  ...
  if (bb) {
    ...
    bbPercentB = {
      ...
      zoneUp: nowB <= 0.05, zoneDown: nowB >= 0.95,
    };
  }
 ...
 * ================================================================================
 * - يعمل على فريم 15 دقيقة ثابت.
 * - المنصة الافتراضية: Binance.
 * - الشراء: 3 نقاط (أوامر Limit).
 * - البيع: 3 نقاط جني أرباح (أوامر Limit).
 * ================================================================================ */

let botState = {
  enabled: false,
  exchange: 'binance',            // المنصة الافتراضية أصبحت Binance
  tradeSizeUsdt: 50,
  takeProfitPercent: 1,
  maxConcurrentPositions: 1,
  positions: {},                  // symbol -> { qty, entryPrice, entryTime }
  pendingOrders: {},              // symbol -> أمر شراء معلق واحد
  pendingSellOrders: {},          // symbol -> مصفوفة أوامر بيع معلقة (3 نقاط)
  tradeLog: [],
  lastSignals: {},
};
...
const TP_LEVELS = [1, 1.5, 2]; // مضاعفات نسبة الربح

// ── دوال حساب النقاط والمؤشرات (كما كانت سابقًا) ──
...
function computeSecondaryScore(indicators, decision) {
  ...
  return trendSign * W.trend + actionSign * W.action + volumeLean * W.volume
       + confidenceLean * W.confidence + trendSign * W.analysis;
}
...
function computeBuyPoints(candles, indicators, sig) {
  ...
  return [
    {
      point: 1,
      label: 'نقطة الهبوط المبكر المكتمل',
      price: roundPrice(price * 0.995),
      triggered: sig.action === 'buy' && sig.passesFilters,
    },
    {
      point: 2,
      label: 'ارتداد من بولينجر السفلي + StochRSI',
      price: roundPrice(bbLower || price * 0.99),
      triggered: sig.action === 'buy' && indicators.stochRsi?.crossUp,
    },
    {
      point: 3,
      label: 'كسر قاع 20 شمعة بابتلاع صاعد',
      price: roundPrice(recentLow * 0.99),
      triggered: sig.action === 'buy' && indicators.candleCompare?.bullEngulf,
    },
  ];
}

// ── تنفيذ الأوامر: MEXC و Binance ──
...
// ── بيع يدوي (يُلغي كل أوامر البيع المعلقة) ──
async function executeBotSell(symbol, sig, reasonOverride) {
  ...
  if (botState.pendingSellOrders[symbol]) {
    for (const o of botState.pendingSellOrders[symbol]) {
      try { await cancelOrder(o.exchange, symbol, o.orderId); } catch {}
    }
...
// ── وضع ثلاث نقاط بيع (Limit) بعد تنفيذ الشراء ──
async function placeBotTakeProfitSells(symbol, position) {
  ...
  for (const [i, mult] of TP_LEVELS.entries()) {
    ...
    try {
      ...
      if (data.orderId) {
        botState.pendingSellOrders[symbol].push({
          orderId: data.orderId, side: 'SELL', price: sellPrice, qty: qtyPerOrder,
          placedAt: Date.now(), exchange: botState.exchange, point: pointName,
        });
        botState.tradeLog.unshift({
          time: Date.now(), symbol, type: 'order', side: 'SELL', price: sellPrice, qty: qtyPerOrder,
          exchange: botState.exchange, reason: `أمر بيع ${pointName} معلّق عند ${sellPrice}`,
        });
        botState.tradeLog = botState.tradeLog.slice(0, 50);
      }
    } catch (err) {
...
// ── أمر شراء معلق (Limit) حسب النقطة المناسبة ──
async function placeBotLimitBuy(sig) {
  ...
  try {
    ...
    botState.tradeLog.unshift({
      ...
      reason: `أمر شراء محدّد (${buyPoint?.label || 'نقطة عامة'}) عند ${buyPrice} — ${sig.filterReason}`,
    });
    botState.tradeLog = botState.tradeLog.slice(0, 50);
  } catch (err) {
    logBotError(symbol, err);
...
// ── فحص الأوامر المعلّقة (شراء + بيع) ──
async function checkPendingOrders(symbol) {
  // فحص أمر الشراء المعلق
  ...
  if (buyOrder) {
    ...
    if (status === 'FILLED') {
      ...
      botState.tradeLog.unshift({
        ...
        quoteAmount, exchange: buyOrder.exchange, reason: `نفذ أمر الشراء عند ${fillPrice.toFixed(6)}`,
      });
      botState.tradeLog = botState.tradeLog.slice(0, 50);
    ...
    for (const order of [...sellOrders]) {
      ...
      if (data.status === 'FILLED') {
        ...
        botState.tradeLog.unshift({
          ...
          quoteAmount, exchange: order.exchange, reason: `نفذ بيع ${order.point} عند ${fillPrice.toFixed(6)}`, pnl, pnlPct,
        });
        botState.tradeLog = botState.tradeLog.slice(0, 50);
...
// ── دورة البوت التلقائية ──
async function runBotCycle() {
  if (!botState.enabled) return;

  const symbolsToCheck = new Set([
    ...Object.keys(botState.pendingOrders),
    ...Object.keys(botState.pendingSellOrders),
  ]);
  for (const symbol of symbolsToCheck) {
    try { await checkPendingOrders(symbol); } catch (err) { logBotError(symbol, err); }
  }
...
// ── تقييم إشارة البوت (تتضمن نقاط الشراء) ──
...
function botStatusPayload() {
  return JSON.stringify({
    ...
    pendingSellOrders: botState.pendingSellOrders,
...
setInterval(runBotCycle, 60 * 1000); // كل دقيقة
